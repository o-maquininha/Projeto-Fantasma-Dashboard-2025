...
quadro1 <- relatorio_vendas %>% 
  summarise(
    ...
    `Desvio Padrão` = sd(peso_kg),
    Variância = var(peso_kg),
    Mínimo = min(peso_kg),
    Máximo = max(peso_kg),
  ...
  summarise(
    Média = mean(altura_cm),
    Mediana = median(altura_cm),
    `Desvio Padrão` = sd(altura_cm),
    Variância = var(altura_cm),
...
tabela9 <- data.frame(
  `Média amostral` = c(p_media, h_media),
  `Variância amostral` = c(p_sd^2, h_sd^2)
) %>%
...
\ \ \ \ Para realizar a análise, primeiro foi verificado a suposição de normalidade das variáveis **peso** e **alutra** analisando a comparação gráfica entre a densidade empírica observada e a densidade teórica de uma distribuição normal, com parâmetros méida $\mu = \bar{x}$ e variância $\sigma^2 = s^2$, estimados separadamente para cada variável. Foram utilizados os valores de $\bar{x}_{peso}$ e $s^2_{peso}$ para a variável peso, e de $\bar{x}_{altura}$ e $s^2_{altura}$ para a variável altura (ver Quadros 1 e 2).

\ \ \ \ Além disso, a suposição de normalidade foi avaliada também por meio da análise dos quantis, comparando os quantis observados com os quantis teóricos da distribuição normal.
...
relatorio_vendas %>% 
  ...
  geom_histogram(aes( y = ..density.., fill = "Distribuição empírica"),
...
qqnorm(relatorio_vendas$peso_kg, ylab = "Quantis amostrais",
       xlab = "Quantis teóricos",
       main = NULL)
qqline(relatorio_vendas$peso_kg, col = "#A11d21", lwd = 2)
```
...
qqnorm(relatorio_vendas$altura_cm, ylab = "Quantis amostrai",
       xlab = "Quantis teóricos",
       main = NULL)
qqline(relatorio_vendas$altura_cm, col = "#A11D21", lwd = 2)
```


\ \ \ \ Em seguida, após a verificação da normalidade das variáveis, foi realizado o teste de correlação de Pearson ao nível de significância de 5% sob as seguintes hipóteses.
...
\ \ \ \ Sob $H_0$, foi obtido um p-valor $< 2.2 \times 10^{-16}$, indicando evidência significativa de correlação linear entre as variáveis. O estimador do coeficiente de correlação, o p-valor, a estatística do teste e o intervalo de confiança a 95% estão na seguinte tabela.
